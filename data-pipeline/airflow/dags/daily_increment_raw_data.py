...
with DAG(
    ...
    with TaskGroup("load_transform_cleanup", tooltip="Load, Transform and Cleanup tasks") as load_transform_cleanup:
        ...
        load_data = BigQueryInsertJobOperator(
            ...
            configuration={
                "load": {
                    "sourceUris": [
                        "gs://naver-placeid-crawler-data-lake/{{ macros.ds_format(ds, '%Y-%m-%d', '%Y/%m/%d') }}/*.parquet"],
                    "destinationTable": {
